...
class AICoachDemo:
    ...
    async def _training_mode_initialization(self):
        ...
        fitness_evaluator = PromptFitnessEvaluator(self.claude_client, training_data)
        ...
        self.coach_analyzer = AICoachAnalyzer(self.claude_client, evolved_prompts)
    ...
    async def _process_data_chunk(self, data_chunk: pd.DataFrame, chunk_id: int):
        ...
        for user_id in data_chunk['user_id'].unique():
            ...
            if nudge:
                ...
                # Log the interaction
                self._log_coaching_interaction(user_id, user_persona, nudge, outcome)
    ...
    def _log_coaching_interaction(self, user_id: int, persona: str, nudge: Dict, outcome: Dict):
        ...
        if outcome['accepted']:
            print(f"   Impact: +{outcome['productivity_impact']:.1%} productivity, +{outcome['satisfaction_impact']:.1%} satisfaction")
    ...
    def _calculate_effectiveness_score(self, nudge: Dict, outcome: Dict) -> float:
        ...
        impact_score = (outcome['productivity_impact'] + outcome['satisfaction_impact']) * 0.6
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='AI Coach Proof-of-Concept System')
